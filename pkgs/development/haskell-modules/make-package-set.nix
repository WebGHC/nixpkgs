  ...
  haskellSrc2nix = { name, src, sha256 ? null, extraCabal2nixOptions ? "" }:
    ...
    in pkgs.buildPackages.stdenv.mkDerivation {
      ...
      installPhase = ''
        export HOME="$TMP"
        mkdir -p "$out"
        cabal2nix --compiler=${self.ghc.haskellCompilerName} ${stdenv.lib.optionalString (!hostPlatform.isWasm) "--system=${hostPlatform.system}"} ${sha256Arg} "${src}" ${extraCabal2nixOptions} > "$out/default.nix"
      '';
  };
