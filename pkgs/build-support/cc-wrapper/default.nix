  ...
  dynamicLinker =
    if !nativeLibc then
      (if targetPlatform.system == "i686-linux"     then "ld-linux.so.2" else
       ...
       # TODO: Handle that this can have a -armhf suffix on some
       # versions. Previously, there was a wildcard to solve this, but
       # that just resulted in empty wildcard expansion on the command
       # line.
